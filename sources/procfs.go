...
func (s *lustreSource) Update(ch chan<- prometheus.Metric) (err error) {
	...
	for _, metric := range s.lustreProcMetrics {
		...
		for _, path := range paths {
			...
			case "job_stats":
				err = s.parseJobStats(metric.source, "job_stats", path, metric.helpText, func(nodeType string, jobid string, jobOperation string, nodeName string, name string, helpText string, value uint64) {
...
func getJobStatsByOperation(jobBlock string, jobID string, operation string) (metricMap map[string]map[string]string, err error) {
	opRegex, err := regexp.Compile(operation + ": .*")
	if err != nil {
		return nil, err
	}
	...
	metricMap["job_id_"+operation+"_samples"] = map[string]string{"help": samplesHelp, "value": opNumbers[0], "jobID": jobID, "operation": operation, "name": "job_id_" + operation + "_samples"}
	metricMap["job_id_"+operation+"_minimum"] = map[string]string{"help": minimumHelp, "value": opNumbers[1], "jobID": jobID, "operation": operation, "name": "job_id_" + operation + "_minimum"}
	metricMap["job_id_"+operation+"_maximum"] = map[string]string{"help": maximumHelp, "value": opNumbers[2], "jobID": jobID, "operation": operation, "name": "job_id_" + operation + "_maximum"}
	metricMap["job_id_"+operation+"_total"] = map[string]string{"help": totalHelp, "value": opNumbers[3], "jobID": jobID, "operation": operation, "name": "job_id_" + operation + "_total"}
...
func (s *lustreSource) parseJobStats(nodeType string, metricType string, path string, helpText string, handler func(string, string, string, string, string, string, uint64)) (err error) {
	metricMap := make(map[string]map[string]string)
